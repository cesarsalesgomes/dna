...
3. The script will get the queries & mutations under the folders `features` and `system`, two different graphql endpoints on the Directus enviroment; and generate the results below (**Obs:** the files will be automatically copied and moved by the script to their usage directories on the Nest/React enviroments):
   ...
   5. `directus.mock.ts`: mocks of the queries/mutations under the **features/system** folders, being used in the Nest environment.

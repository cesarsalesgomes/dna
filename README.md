...
3. To use on cloud enviroment, copy the schema generated to your cloud Directus folder and run **npx directus schema apply /path/to/schema.yaml**.
...
3. The script will get the queries & mutations under the folders `features` and `system`, and generate the results below (**Obs:** the files will be automatically copied and moved by the script to their usage directories on the Nest/React enviroments)

   1. `directus.sdk.ts`: operations of the queries/mutations under the **features** folder, being used in the React/Nestjs environments.

   2. `directus-system.sdk.ts`: operations of the queries/mutations under the **system** folder, being used in the React/Nestjs environments.

   3. `directus.hooks.ts`: hooks of the queries/mutations under the **features** folder, being used in the React enviroment.

   4. `directus-system.hooks.ts`: hooks of the queries/mutations under the **system** folder, being used in the React environment.

   5. `directus.mock.ts`: mocks of the queries/mutations under the **featues/system** folders, being used in the Nest environment.

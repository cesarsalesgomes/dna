...
1. Fork this repository to start a new project based on the `Dna` architecture (**[Github fork guide](https://docs.github.com/get-started/quickstart/fork-a-repo#configuring-git-to-sync-your-fork-with-the-original-repository)** to sync the fork with the `Dna` repository).
...
3. The script will look for all folders containing a `codegen.yml` configuration file, and then generate the 3 files below based on the queries/mutations within each feature:
   ...
   1. `folder_nameDK.ts`: operations of the queries/mutations to be used in the React/Nest environments.
